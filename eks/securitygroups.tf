locals {
  common_sg_tags = {
    "kubernetes.io/cluster/${aws_eks_cluster.control_plane.id}" = "owned"
    "aws:eks:cluster-name" = aws_eks_cluster.control_plane.id
    "Name" = "eks-cluster-sg-${aws_eks_cluster.control_plane.id}"
  }
